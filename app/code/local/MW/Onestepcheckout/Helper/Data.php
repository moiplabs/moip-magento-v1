...
class MW_Onestepcheckout_Helper_Data extends Mage_Core_Helper_Abstract
{
	...
	public function onlyProductDownloadable(){		//neu chi co duy nhat 1 loai san pham downloadable
	...
	public function haveProductDownloadable(){		//neu co san pham downloadable trong cart
		$itemProduct=$this->getQuote()->getAllVisibleItems();
		foreach($itemProduct as $item){
	...
	public function issubscribed(){
		...
		if(!Mage::getSingleton('customer/session')->isLoggedIn() or !$issubscribe){
			return true;  //cho hien thi subscribe neu customer chua login or customer ko dang ky subscribe
		}
		else{
			return false;
		}
